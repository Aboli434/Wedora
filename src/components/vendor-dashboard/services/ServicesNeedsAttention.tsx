...
import { getServicesNeedingAttention } from "@/lib/vendorServices";
import { AlertCircle, CheckCircle2, ArrowRight } from "lucide-react";

interface ServicesNeedsAttentionProps {
  services: VendorService[];
  onSelectService: (service: VendorService) => void;
}

export function ServicesNeedsAttention({ services, onSelectService }: ServicesNeedsAttentionProps) {
  const issues = getServicesNeedingAttention(services);
    ...
    <section className="bg-[#FAF8F5] border border-[#E8E2D9] rounded-xl p-6 md:p-8 space-y-4">
      <div className="flex items-center justify-between border-b border-[#E8E2D9] pb-4">
        <div>
          ...
          <p className="text-xs text-[#6E6B65] mt-0.5">
            Optimize your service offerings for better couple engagement.
          </p>
        </div>
      ...
      {issues.length === 0 ? (
        <div className="flex items-center gap-3 p-4 rounded-lg bg-emerald-50/80 border border-emerald-200 text-xs text-emerald-950 font-medium">
          ...
          <span>Your services are ready to be discovered on Wedora.</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {issues.map(({ service, reason }, idx) => (
            <div
              key={`${service.id}-${idx}`}
            ...
            >
              <div className="flex items-center gap-2.5 pr-2">
                ...
                <div>
                  <p className="font-medium text-[#2C2A29]">{service.name}</p>
