...
import { validateBookingNotes } from "@/lib/vendorBookings";
...
export const BookingNotes: React.FC<BookingNotesProps> = ({
  ...
  const handleSave = () => {
    const val = validateBookingNotes(text);
    if (!val.valid) {
      setError(val.error || "Invalid note length.");
      return;
    }
    setError(null);
    onSaveNotes(text);
        ...
        <div>
          {notes ? (
            ...
            <p className="text-xs text-[#5A5650] italic mb-3">
              No internal notes added yet. Track crew assignments, logistics permits, or special client requests here.
            </p>
          )}
