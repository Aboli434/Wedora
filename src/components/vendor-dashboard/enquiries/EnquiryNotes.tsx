...
import { validateEnquiryNotes } from "@/lib/vendorEnquiries";
...
export const EnquiryNotes: React.FC<EnquiryNotesProps> = ({
  notes = "",
  onSaveNotes,
  onDeleteNotes,
}) => {
  ...
  const handleSave = () => {
    const val = validateEnquiryNotes(text);
    if (!val.valid) {
      setError(val.error || "Invalid note format.");
      return;
    }
    setError(null);
    onSaveNotes(text);
    ...
    <div className="bg-white border border-[#161514]/10 p-5 mb-8">
      ...
      {!isEditing ? (
        <div>
          {notes ? (
            ...
            <p className="text-xs text-[#5A5650] italic mb-3">
              No internal notes added yet. Use notes to track crew availability, venue requirements, or negotiation history.
            </p>
          )}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs border border-[#161514]/20 font-medium hover:bg-[#FAF8F5] transition-colors"
            >
              {notes ? (
                <>
        ...
        <div className="space-y-3">
          <textarea
            ...
            placeholder="Add internal vendor notes..."
      ...
      {showConfirmDelete && (
        <div className="mt-3 p-3 bg-rose-50 border border-rose-200 text-rose-900 text-xs flex items-center justify-between gap-3">
          <span>Are you sure you want to delete this internal note?</span>
          <div className="flex items-center gap-2">
            <button
