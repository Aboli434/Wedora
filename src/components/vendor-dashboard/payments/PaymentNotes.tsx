...
import { MessageSquare, Trash2, Edit2, ShieldAlert } from "lucide-react";

interface PaymentNotesProps {
  paymentId: string;
  notes?: string;
  onSaveNotes: (paymentId: string, content: string) => void;
...
export const PaymentNotes: React.FC<PaymentNotesProps> = ({
  ...
  const handleSave = () => {
    if (noteContent.trim()) {
      onSaveNotes(paymentId, noteContent.trim());
      setIsEditing(false);
    }
  };
      ...
      {(isEditing || !notes) && !confirmDelete && (
        <div className="space-y-2">
          <textarea
            ...
            placeholder="Add internal vendor note (e.g. Bank reference, payment arrangement, follow-up date)..."
